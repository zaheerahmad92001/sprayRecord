...
import styles from '../EditReturnProduct/styles';
...
export default class EditReturnProducts extends Component {
    constructor(props) {
        super(props);
        this.item = this.props.navigation.getParam('pData');
        console.log('props',this.item)
        this.state = {
            qty:this.item.qty, isDatePickerVisible: false,
            date: this.item.date, errorMsg: '',
            weight: this.item.weight,
            weightUnit: this.item.unit,
            selected:this.item.name, mapOnce: true,
            populate: [],buttonDisabled: true,
        }
    }
    ...
    saveInfo = () => {
        const { date, qty, weight, weightUnit, selected ,buttonDisabled } = this.state;
    ...
    render() {
        const {selected, date, qty, errorMsg ,buttonDisabled } = this.state;
        {
            this.state.mapOnce ?
            myProduct.map((value) => {
        ...
        return (
            <Container style={styles.container}>
                <_Header
                    ...
                    HeadingText={'Edit Return Product'} />
                <Content style={styles.content}>
                    ...
                    <View style={{ marginBottom: RFValue(10), marginTop: RFValue(35) }}>
                        ...
                        <View style={{ borderWidth: 1, borderColor: BBCOLOR, borderBottomRightRadius: 15, borderTopLeftRadius: 15 }}>
                            <RNPickerSelect
                                ...
                                onValueChange={(value)=>this.setState({ selected: value, errorMsg: '',buttonDisabled:false })}
                    ...
                    <View style={styles.Input}>
                        <Text_Input
                            ...
                            placeholderTextColor={'#979797'}
                            onChangeText={(value) => this.setState({ qty: value, errorMsg: '',buttonDisabled:false })}
                            value={qty}
